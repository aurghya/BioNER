...
BERT_PRETRAINED_DIR = '../biobert_v1.1_pubmed'
...
f = open('converted_train.txt')
f2 = open('embeddings.txt', 'w')
text = f.readlines()
for i,line in enumerate(text):
  sent, tags = line.split('\t')
  print(sent)
  emb = get_features([sent])
  ...
  for key, val in emb.items():
    ...
    if key[0]=='#':
      ...
      tags.append(tag_words[j])
      j=j+1
  tmp = {}
  tmp['words'] = words
  tmp['embeddings'] = embeddings
  tmp['tags'] = tags
  json.dump(tmp,f2)
  f2.write('\n')
